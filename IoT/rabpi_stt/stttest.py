...
def sense():
    ...
    while True:
        ...
        while True:
            ...
            if uw > 100:
                print("remote")
                exit_event.set() # Set the event to signal that recording should stop
                time.sleep(1)
            else:
                print("close")
                if exit_event.is_set():
                    exit_event.clear() # Clear the event to signal that recording should start
                time.sleep(1)
...
def rec():
    ...
    try:
        while not exit_event.is_set(): # Check if the event is set
            data = stream.read(CHUNK)
            frames.append(data)
            print(exit_event.is_set(), end=" ")
